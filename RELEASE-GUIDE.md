...
For the new automated GitHub Actions / Docker Desktop workflow, follow
[CI-CD-SETUP.md](CI-CD-SETUP.md). The sections below describe the earlier manual
release process; use the automated guide's persistent image override after adopting
the pipeline, and do not mix image-selection procedures.
...
This guide uses Bash on Linux, WSL, or Git Bash, with Docker Compose, Git, tar,
and curl available. Commands are instructions for you to run, not actions already
performed. Use a checkout containing the development configuration and backend
test script described below; those changes are not yet present on every branch.

For Git Bash on Windows, disable automatic conversion of container paths for this
session before running Docker commands:

```bash
export MSYS_NO_PATHCONV=1
```

WSL and Linux do not need that setting. Keep all deployment commands in the same
shell session so paths and variables remain consistent.
...
Choose a time when nobody is entering game records. Keep this Bash session
...
releaseStamp="$(date +%Y%m%d-%H%M%S)"
prodCheckout="$(pwd -P)"
prodContainer="$(docker compose -f docker-compose.yml ps -q poker-tracker)"
previousImage="$(docker inspect --format '{{.Image}}' "$prodContainer")"
...
```bash
releaseWorkspace="$(mktemp -d)"
releaseSource="$releaseWorkspace/source"
releaseArchive="$releaseWorkspace/source.tar"
mkdir "$releaseSource"
...
```bash
backupRoot="$(dirname "$prodCheckout")/poker-tracker-backups"
backupDir="$backupRoot/$releaseStamp"
umask 077
mkdir -p "$backupRoot"
mkdir "$backupDir"
...
```bash
cp -a "$prodCheckout/data" "$backupDir/"
sha256sum "$prodCheckout/data/poker.db" "$backupDir/data/poker.db"
printf '%s\n' "Release commit: $releaseCommit" "Previous image: $previousImage" "Rollback tag: $rollbackImage" > "$backupDir/release.txt"
```
...
```bash
releaseOverride="$backupDir/release.compose.yml"
printf '%s\n' 'services:' '  poker-tracker:' "    image: $candidateImage" > "$releaseOverride"
...
curl --fail --show-error --silent --include 'https://www.carsonsgame.com/api/health'
```
...
```bash
rollbackOverride="$backupDir/rollback.compose.yml"
printf '%s\n' 'services:' '  poker-tracker:' "    image: $rollbackImage" > "$rollbackOverride"
...
commands until the next release. If this is a new Bash session, recover the
